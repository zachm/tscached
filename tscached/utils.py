...
def get_chunked_time_ranges(config, time_range):
    ...
    """
    chunk_length = config['chunking'].get('chunk_length', 1800)
    num_chunks = config['chunking'].get('max_chunks', 6)
    start_time, end_time = get_needed_absolute_time_range(time_range)
    if not end_time:
    ...
    if elapsed_secs <= chunk_length:
        ...
        for i in xrange(num_chunks):
            start = end_todo - length_td

            # Sanity check: make sure we limit the earliest chunk if it's partial
            if start < start_time:
                start = start_time

            logging.info((start, end_todo))
            chunks.append((start, end_todo))
            end_todo -= length_td
        return chunks
